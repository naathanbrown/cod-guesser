...
There is a daily: ten launch maps, the same set for everyone on that date. Remake mode shows a map that came back and asks which game version it is. Custom matches still use the game filters, launch vs full locker, loading screen or minimap, four choices or typed answers, and 5 / 10 / 15 / unlimited rounds. Intel reveals the game and cuts that round's score in half. The earliest games have almost no minimaps on the wiki, so minimap mode skips them. Printed map names are covered when OCR can read them.
